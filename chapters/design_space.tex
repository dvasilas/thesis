...
The read path evaluates this query by scanning all images in $photoAlbum$.
...
\begin{tabular}{M{0.2\linewidth-\tabcolsep}|M{0.8\linewidth-\tabcolsep}@{}}
Fast path & Lookup on indexed attribute: Fast lookup (no scan needed). \\ \hline
Slow path & Lookup on non-indexed attribute: Fallback to other. \end{tabular} \\ \hline
...
An alternative approach is to place index replicas on a subset of the data centers (Figure~\ref{fig:index_partial_replication}).
