...
The literature often refers to this scheme as \text{sync-insert}.
...
Another asynchronous policy, termed \text{async-simple} consists of acknowledging the write operation to the client as
...
% Changes to the table data are propagated to the global secondary indexes within a fraction of a second, under normal conditions.
% However, in some unlikely failure scenarios, longer propagation delays might occur.
% Because of this, your applications need to anticipate and handle situations where a query on a global secondary index returns results that are not up to date.
