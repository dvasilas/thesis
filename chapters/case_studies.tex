...
We focus on Zenko's federated metadata search functionality.
...
\todo{change figure to Zenko overview}
\todo{cite zenko docs for architecture}.
Zenko uses a warehousing approach to provide federated metadata search:
It integrates metadata tags for objects stored on all storage platforms in a central \textit{metadata store}.
...
Queries on metadata tags are translated to MongoDB find requests \todo{cite}.

A typical Zenko deployment consists of Zenko deployed on-premises, along with a private storage system,
and multiple public cloud storage platforms, each on a different data center.
For this case study, we refer to each data center / storage system as a \todo{storage location}.
...
Zenko ingests updates performed directly to an underlying storage system through a mechanism called out-of-band updates \todo{cite}.

\medskip
\noindent
In this case study,
we present an alternative, decentralized approach for achieving federated metadata search on object storage platforms.
More specifically, we demonstrate how the QPU-based composable architecture can be used to construct a federated secondary
index over data stored on multiple storage locations.
The main idea is to partition the index by storage location,
and place each index partition close to the underlying data.
...
Second, a corpus driver QPU, as described in chapter \todo{ref to chapter} is available for each storage platform.

\medskip
\noindent
The QPU architecture for the multi-cloud index is shown in Figure~\todo{figure}.
...
storage location.
To address this issue we address the multi-cloud index architecture as shown in Figure~\todo{ref}.
This architecture introduces the Remote Index QPU class.
We define the Remote Index that combines the properties of an Index and a Cache QPU.
The Remote Index materializes only a bounded subset of its index entries,
using a least recently used policy to discard entries.

When initially deployed, the Remote Index is empty.
When it receives a query, its query processing function forwards the query to its downstream connection,
which in this architecture is an index location, as a persistent query.
The QPU then stores the corresponding index entries, and keeps them up-to-date using the established input stream.
It processing subsequent queries using the cached index entries.
To achieve that ...
When a query requires non-materialized index entries and there is no available space,
the least recently used entries are discarded until there is enough space for the queried entries.

\medskip
\noindent
Using the Remote Index QPU reduces the cross data center communication required for query processing by
replicating recently accessed index entries.
However, index maintenance might still incur significant between storage locations,
in cache index entries materialized in the Remote Index are frequently updated.
To address that,
we extend the Remote Index QPU with a mechanism for measuring the rate of updates to each index entry.
If the rate for a certain entry crosses a threshold specified by the QPUs configuration,
then the QPU stops receiving update for this entry and discards it.
To achieve that ..
For example ..

\medskip
\noindent
Both functionalities provide fine-grained placement of index entries.

This case study demonstrates an important property of the proposed query processing architecture.
A QPU-based query processing architecture,
can be viewed as a hierarchical architecture, composed of a number of distinct tiers;
Each tier


\subsection{Need title inspiration}

In this section, we extend the multi-cloud index case study by examining a use case inspired by one a Scality's customers.
We consider the example of a media organization that operates broadcasting and streaming services.
The media assets (video content, images) used by these services on are stored in public cloud storage platform,
and are tagged with metadata attributes.
Search on metadata tags is used to support recommendation service and user search functionalities.
Moreover, the organization operates in two geographic locations;
A separate storage platform is used for each location.

The metadata search requirement of this use case are as follows:
Applications on each geographic location need low latency metadata search on \textit{local} data (data stored on that geographic location),
as well as a \textit{subset} of the \textit{remote} data.
This subset is specified by the application, and can be expressed as predicates on metadata tags.
In other words, for an application that operates in $location_A$, there is an ``interest set'' that is a subset of the corpus
located in $location_B$ that the application accesses frequently and needs low latency metadata search on.

The state-of-the-art warehousing approach of constructing a central index with metadata tags from all objects
on both locations, can be applied to achieve the required functionality.
However, this approach uses more resources that required, as it includes the entire set of remote data.
Our goal in this use case it to enable a secondary index that contains a subset of a data corpus, based on a given predicate.

\medskip
\noindent
Figure \todo{figure} shows how we can achieve this functionality using a QPU-based architecture.
We use a layer of Selection QPU between the corpus (Corpus Driver QPUs) and the partitioned index.
The Selection QPUs filter that corpus, and make the data items that satisfy the specified query available to the index.
We refer to this architecture, as an \textit{filtered index}.
...
A federated index for this use case consists of two parts:
A partitioned index, as shown in section \todo{ref}, responsible for the \textit{local} corpus,
and a filtered index,
responsible for the ``interest set'' of the \textit{remote} corpus.
