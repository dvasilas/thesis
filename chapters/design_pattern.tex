...
\medskip
\noindent
The design of a geo-distributed query processing system presents the several unique design issues:
\begin{itemize}
...
\begin{itemize}
  ...
  load balancing, and federation of independent corpora.
\end{itemize}

\medskip
\noindent
Second, each of these types of functionalities can be encapsulated by a \textit{system component} with microservice semantics:
...
and returns the items that satisfy the given condition.

The structure of the service interface's response is common for the three types of components:
...
Components that maintain derived state structures, and relational operators such as aggregations, are stateful.
Other components, such as filters are stateless.
However, stateful components only require access to \textit{internal} state, and don't require shared state.
For example, a partitioned index (Figure~\ref{fig:observations_partitioned_index}) can be expressed as a set of ``index'' components representing index partitions.
...
In summary, our second observation is that the three types of query processing functionalities can be encapsulated
by a stateful or stateless microservice-like component that provides its functionality through an interface.
The response to an invocation of this interface is a set of data items.
...
\begin{itemize}
  ...
  index partition components, receives as input the corresponding index lookup results (Figure~\ref{fig:observations_partitioned_index}).
\end{itemize}

We can categorize inputs in two types:
\begin{itemize}
  \item An index or materialized view requires \textit{updates} representing information about modifications of the corpus as input.
  Moreover, need to continuously receive this input throughout their operation,
  in order to remain up-to-date with the corpus.

  \item Every other query processing functionality requires a set of data item \textit{states} as input.
  Contrary to indexes and materialized view, the scope of this input is a \textit{single query or request}.
  For example, a filter operator receives a set of data item as input that is tied to specific execution.
\end{itemize}

Our third observation is that these two types of input can be unified by representing the input
that a query processing functionality requires as a \textit{stream of records}, structured as data item state deltas.
First, both types of inputs can be represented using streaming semantics.
The input required by an an index or materialized view can be viewed as a continuous stream of updates.
The input required by other query can be viewed as a stream of data item states.
Second, a data item state delta, consisting of a base state and a modification (or the new state after the modification is applied)
can represent either an update to the state of a data item, or the state itself (by leaving empty the second part of the delta)

Using this observation we can extend the query processing component described above with a ``port'' for
receiving an input stream consisting of records structured as data item state deltas.

\medskip
\noindent
Our forth observation is that the two characteristics of the query processing component,
the service interface and the input port,
are not independent from each other.
An stream being received as an input by a component,
...
\medskip
\noindent
Our fifth observation is that while each individual component encapsulates a basic query processing functionality,
...
This is the case in the partitioned index example, in which derived state components (index partitions), and a routing component
(partition manager) are composed to implement a partitioned index.
