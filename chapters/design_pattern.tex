...
Moreover, this materialized view is partitioned with the partition by document approach, using the $story_{id}$ as partitioning key.
...
\begin{algorithmic}
\Function{ProcessQuery}{queryReq, qpState, [downstreamConn], outputStream}
\State [indexTermL, indexTermB = build terms for reading secondary index from queryReq
\State result = qpState.Get(indexTermL, indexTermB)
\State outStream.Send(result)
\EndFunction
\end{algorithmic}
\end{algorithm}
...
QPU-based query processing systems execute a similar type of computations for incrementally updating
