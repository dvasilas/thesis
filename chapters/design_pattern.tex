...
In chapter~\ref{ch:models} \todo{MUST HAVE: consistency with model chapter}, we have modeled a data item of a corpus table as tuple of attributes.
...
\begin{itemize}
  ...
  % \todo{interval is simply a predicate}
...
\begin{lstlisting}[
  basicstyle=\normalfont\ttfamily,
  caption={Initialization function signature},
  captionpos=b,
  label={lst:init_func}
  ]
...
\begin{lstlisting}[
  basicstyle=\normalfont\ttfamily,
  caption={Input stream callback function signature},
  captionpos=b,
...
\todo{change merge to rule based approach: go over corpus driver, index, partition, ... to explain how a QPU computes its domain tree based on those of its connections}
