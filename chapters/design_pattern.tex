...
Existing query processing system provide some tunning capabilities, for example index and materialized view selection,
and provide mechanisms for adjusting query execution to individual queries (query optimization).

However, in the context of modern, global-scale systems, in which clients and data are geo-distributed, the
\textit{placement} of derived state (index, materialized views) creates additional trade-offs and magnifies the effects
of existing trade-offs.
Existing query processing systems do not provide mechanisms for configuring the placement of derived state.


\medskip
\noindent
The goal 
(The proposed approach is based on the following requirements):
\begin{itemize}

  \item \textbf{Tunability.}
  Achieving efficient query processing requires the ability to adjust the query processing system
  to the workload characteristics, data and access distribution patterns, and requirements of specific use cases.
  An efficient query processing system thus needs able to be configured along the following dimensions:
  ...
  \begin{itemize}
    ...
    of minimizing the inter-site communication in the read path,
    and minimizing the inter-site communication in the write path.
    We have analyzed the implication of this fundamental trade-off in section~\ref{sec:topology_patterns}.
    ...
    the overhead incurred to write operations and the consistency between derived state and corpus data.
    ...
    and work on the write path as well as the resource required for maintaining derived state on the other side.
    ...
    work framework.
    Therefore, design decisions about caching should be unified under the same tunning mechanisms as index and materialized
    view selection.
  ...
  \item \textbf{Independence from corpus distribution schemes.}
  ...
  We therefore require our design to not be based not be based on specific assumptions the corpus distribution scheme.
...
We propose a \textit{framework} for constructing query processing systems in a case-by-case basis.
...
We view the query processing system as a \textit{middleware system} that communicates with the storage tier through
well-defined interfaces.
This provides the query processing system with the flexibility required to enable configurable state and computation placement
and to support diverse data distribution schemes.
...
\medskip
\noindent
The second decision is to not design a \textit{specific} query processing system,
...
The proposed framework provides a set of composable, configurable building blocks that encapsulate simple query processing tasks
such filters, aggregations, and joins, as well as derived state structures, including secondary indexes, materialized views.
...
Building blocks are independent components with well-defined boundaries.
Because of that, their placement across the system is orthogonal to their functionality.
This addresses the requirement for flexible state and computation placement.
...
\bigskip
\noindent
Our modular query processing architecture design is based on a set of \textbf{observations} about the
functionality and semantics of query processing systems.
...
We unify the semantics of these query processing primitives under a component that can be used as the building block
...
\begin{itemize}
  \item The execution of relational algebra operations, such as filters, aggregations, and joins.
  \item Maintaining derived state structures (secondary indexes, materialized view, and caches).
  These are used to facilitate the execution relational algebra operations.
  \item ``Routing'' functionalities, such as the management of access to partitioned derived state,
  and load balancing.
\end{itemize}

\medskip
\noindent
Second, each of types of functionalities can be encapsulated by a system component with microservice semantics:
...
For example, a secondary index can be encapsulated by a component with an interface which maps to the index's
lookup operation.
A filter operator can be encapsulated by a component with an interface that receives a set of items and a condition,
and returns the only the items that satisfy the given condition.

The structure of the interface response is common for the three types of components:
A set of data items, each composed of a primary key and a set of secondary attributes.
For example, this can represent the response of a materialized view or cache to a given query,
the result of an index lookup, or the result of a filter operation.

Furthermore, this component might be stateful or stateless.
Components that maintain derived state structure, and relational operators such as aggregations, are stateful.
Other components, such as filters are stateless.
However, stateful components only require access to \textit{internal} state, and don't need to access shared state.
For example, a partitioned index can be expressed as a set of ``index'' components representing index partitions.
The state of each component contains the index entries of the corresponding index partition.
An index lookup can be performed by invoking the interfaces of the relevant index partitions.

\medskip
\noindent
The next observation is related to the \textit{input} required for the three types of query processing functionalities:
\begin{itemize}
  \item Indexes and materialized views expose a second interface for being updated to reflect
  ...
  \item Relational operators receive one or multiple sets of items as input, and perform a transformation on those input
  according to their functionality.
  For example, a join operator receives the data items of two tables as input and performs a join operation on them.

  \item The functionality of routing components consists of invoking the interface of other components,
  and receive as input the corresponding responses.
  For example, given a query, a component responsible for managing access to index partitions invokes the interfaces of
  index partition components, receives as input the corresponding index lookup results.
\end{itemize}

We can categorize these semantics in two types of inputs:
\begin{itemize}
  \item Indexes and materialized views receive as input a \textit{continuous stream of updates},
  throughout their operation.

  \item All other components receive as input a set of data item \textit{states},
  which represents the result of a \textit{single query} or request.
\end{itemize}

Our third observation is that these two types of inputs can be unified by representing the input
to a query processing component as a \textit{stream of data item state deltas}.
First, both types of inputs can be represent as a \textit{stream of records}.
Second, data item state delta $D$ $=$ $\{State_0$, $State_1\}$ can represent either an update to the state of a data item
($State_0$ is the data item's state before the update is applied,
and $State_1$ is the data item's state before the update is applied)
or the state itself
($State_0$ is not used,
and $State_1$ represents the data item's state).

\medskip
\noindent
Combining the second and third observation,
we extend the semantics of the query processing component as follows:
The response to an invocation of the component's service interface is a stream of records structured
as data item state deltas.

\medskip
\noindent
Our forth observation is that while the functionalities of individual components are simple,
the components can be \textit{composed} to implement higher-level query processing functionalities.
This a common technique used in existing query engines,
in which the execution of relational algebra expressions is represented as a dataflow computation performed by a
directed acyclic graph of operators \cite{cockroachdb:distsql, gjengset:noria}.
The graph's components are relational operators that receive one or more input streams,
perform them, and emit an output stream.
Graph nodes are connected so that the output stream of one operator is the input stream of another.

Our final observation is that this composition mechanism can be extended to involve relational operator components as well as
derived state and routing components.
A component $C_a$ can invoke the service interface of another component $C_b$ of any type.
As a result, a $C_b$ initiate a stream with $C_a$ as the receiver.

\bigskip
\noindent
Combining all the observations described above,
we define a \textit{system component} that combines properties of a streaming operator and a microservice.
...
The response to a query request is a stream.
...
Higher-order query processing tasks are achieved by \textit{composing} multiple QPUs.
For example,
a partitioned secondary index can be expressed as a composition set of ``secondary index'' QPUs,
that represent index partitions, along with an ``index partition'' QPU.
\todo{figure example  join, materialized view, }
...
\medskip
\noindent
The computation model of query execution emerges from the semantics of the query processing unit.
...
However each QPU instance specializes in only a subset of the queries that can be expressed by the interface's query
language.
...
The QPU model defines the signatures of these functions, and each QPU class provides implementations for them.
