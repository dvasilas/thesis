...
Figure~\ref{fig:readV_cdf_throughput} displays a cumulative distribution function (CDF) showing how stale a
...
% The evaluation results shown in Figures~\ref{fig:ycsb_responseTime} and~\ref{fig:ycsb_update_latency_local_local_remote} demonstrate the trade-off between
...
Figures~\ref{fig:freshness_throughput_9505} and~\ref{fig:freshness_throughput_5050} show
...
Figures Figure~\ref{fig:freshness_cdf_9505} and~\ref{fig:freshness_cdf_9505} show the CDFs of returned version for
...
\end{itemize}
  % \item For the w50/50 workload, local and remote-corpus return stale results for loads greater than 10k operations/second
...
Furthermore, we observe that using a caching layer results in data transfer reduction for the both workloads, as expected.
